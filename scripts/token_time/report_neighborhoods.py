...
try:
    ...
    from neighborhood_report import (
        format_table,
        neighborhood_rows,
        top_rows,
    )
...
def write_report(
    ...
    *,
    ...
    lines = [
        ...
        "",
        "```text",
        "r_t(w)[v] = cos(centroid_t(w) - mu_t, centroid_t(v) - mu_t)",
        "delta(w) = 1 - cos(r_t0(w), r_t1(w))",
        "std_r_t(w)[v] = standardize_v(r_t(w)[v])  [per-reference z-score; NOT ζ(w)]",
        "delta_std_r(w)[v] = std_r_1(w)[v] - std_r_0(w)[v]",
        "```",
        "",
        f"- layer: `{layer}`",
