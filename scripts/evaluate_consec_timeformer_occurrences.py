...
try:
    ...
    from evaluate_consec_gate3 import (
        jensen_shannon,
        prepare_context,
        rankdata,
        spearman,
...
def partial_spearman(values_a, values_b, control) -> float:
    ranks_a = rankdata(values_a)
    ranks_b = rankdata(values_b)
    ranks_control = rankdata(control)
    design = np.column_stack(
        [np.ones(len(ranks_a), dtype=np.float64), ranks_control]
    )
