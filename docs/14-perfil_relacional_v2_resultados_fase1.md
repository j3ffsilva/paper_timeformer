...
`docs/12-novo_perfil_relacional.md` define o "Perfil Relacional e Trajetória
...
A auditoria do codex (segunda opinião, ver `tmp/29-codex_semantic_modes_v2_nogo_review.md`)
...
### 7.26 Inicialização com `bert-tiny` e treino contínuo (2026-06-12)

Foi implementada a importação de `prajjwal1/bert-tiny` para o
`RealStaticMLM`: duas camadas, `d_model=128`, 2 heads, `d_ff=512`,
pós-normalização, GELU e máscara de padding. Para cada token do vocabulário
SemEval, o embedding inicial é a média dos WordPieces correspondentes
(removendo `_nn`/`_vb`). `pos_emb` e `mlm_head` permanecem aleatórios.

O treino recomendado foi executado integralmente:

- `D0`: 12 épocas, 23.148 passos; loss 6.9455 -> 4.7883;
- `D1`: 8 épocas adicionais, 17.040 passos; loss 5.4283 -> 4.9589;
- total: 40.188 passos, batch 192, LR `1e-4`, cerca de 1h59 na RTX 3060.

**Resultado principal, encoder fixo (n=37):**

| Checkpoint fixo | Layer | APD Spearman (p) | ROC-AUC / AP |
|---|---|---|---|
| init, antes de MLM contínuo | layer_2 | **0.277 (p=0.097)** | 0.628 / 0.625 |
| init, antes de MLM contínuo | mean_last_2 | 0.224 (p=0.183) | 0.640 / 0.592 |
| theta0, após 12 épocas em D0 | layer_2 | -0.055 (p=0.747) | 0.497 / 0.498 |
| theta0, após 12 épocas em D0 | mean_last_2 | -0.016 (p=0.926) | 0.515 / 0.521 |
| theta1, após +8 épocas em D1 | layer_2 | 0.123 (p=0.467) | 0.551 / 0.592 |
| theta1, após +8 épocas em D1 | mean_last_2 | **0.204 (p=0.225)** | 0.548 / 0.575 |

O melhor perfil oculto do checkpoint inicial (`centered_relational_apd`,
`mean_last_2`) chegou a spearman **0.337 (p=0.041)**. Depois do treino,
o melhor perfil diagonal ficou em 0.200 (p=0.236).

Rodamos também o oráculo congelado com o próprio `bert-tiny`. Como ele tem
apenas duas camadas, `mean_last_4` agrega embeddings + ambas as camadas:
APD spearman **0.399 (p=0.014)**, contra 0.093 na última camada isolada.
Logo, o modelo pequeno pré-treinado contém sinal útil, embora menor que o
`bert-base` (~0.59). A adaptação para o vocabulário próprio preserva parte
desse sinal; o MLM contínuo longo com LR `1e-4` remove a maior parte,
especialmente na camada final.

**Conclusão:** esta rodada não confirma ganho da configuração final, mas
também não refuta a inicialização pré-treinada. Ela identifica
**esquecimento durante o fine-tuning contínuo** como o próximo gargalo.
A próxima ablação deve reduzir a agressividade do treino sem usar
`truth.tsv` para seleção: LR `3e-5`/`5e-5`, menos épocas e/ou congelamento
inicial da camada inferior, avaliando checkpoints por época com o
protocolo fixo já definido. Não repetir 12+8 épocas a LR `1e-4` como
configuração principal.

Os modos primeiro também não separaram consistentemente mudança de
estabilidade: `graft_nn` teve JSD 0.506, mas `multitude_nn` (controle)
0.317; `plane_nn` ficou em 0.168. Isso mantém a Tarefa 3 como diagnóstico
qualitativo, não como métrica principal.
...
- `docs/12-novo_perfil_relacional.md` -- documento canônico v2 (§1-14)
- `docs/05-relational_change_current_plan.md` -- log de decisões (seção
...
- `tmp/29-codex_semantic_modes_v2_nogo_review.md` -- auditoria do NO-GO
  (segunda opinião)
- Resultados:
