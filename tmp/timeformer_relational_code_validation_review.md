...
**Baseado em:** leitura integral de `relational.py`, `relational_metrics.py`, `run_relational_continual_sanity.py`, `test_relational.py`, `relational_change_current_plan.md`, `dataset.py` (classes novas), `train.py:ContinualPeriodTrainer`, `config.json`, `counterfactual_summary.csv`, `relational_summary.csv`
...
**Caso C (apenas S1 muda):** O perfil relacional de S1 muda. Os perfis de outros sujeitos também mudam por causa da relação deles com S1. Isso está documentado no plano (`relational_change_current_plan.md`) e é matematicamente correto. A métrica direcional capturará essa mudança corretamente.
